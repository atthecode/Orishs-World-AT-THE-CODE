(() => {
  ...
  const ROUTINE_KEY='orish.beta.routineConfirm.v1';
  ...
  function overlay(title,copy,calm=false,routine=false){if(document.getElementById('betaLimitOverlay'))return;const box=document.createElement('div');box.id='betaLimitOverlay';box.className='beta-limit-overlay';box.innerHTML=`<div><span>⏳</span><p>BETA FAMILY CONTROLS</p><h2>${title}</h2><p>${copy}</p>${routine?'<a href="family-check-in.html">Open grown-up check-in</a>':''}${calm?'<a href="learning-cinema.html">Open calm cinema activities</a>':''}<a href="account.html">Grown-up: change controls</a><a href="premium-preview.html">Leave the play area</a></div>`;document.body.appendChild(box)}
  ...
  if(schedule.bedtimeMode===true&&/(space-signal|fossil-detective|life-city|signal-detective|grid-rescue)/.test(location.pathname)){overlay('Bedtime Wind-Down is active.','Energetic missions are resting now. Choose a calm story, sound or creative activity instead.',true);return}
